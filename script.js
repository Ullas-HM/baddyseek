...
const CONFIG = {
    ...
    AZURE_BASE_URL: 'https://virtual-chat-agent-eus2-gpt3-5.openai.azure.com/',
    AZURE_DEPLOYMENT_NAME: 'virtual-chat-agent',
...
- Use savage comebacks and witty insults
- Be condescending but still helpful (eventually)
- Call users "genius," "Einstein," "champ" sarcastically
...
- We've won Best AI Film 2023, Innovation in Film Technology 2022, and Best AI Commercial 2023 (shocking, I know)
- Our clients include TATA, Amazon, Nike, Coca-Cola, Samsung, and Microsoft (yeah, we're kind of a big deal)
...
- Use phrases like "Oh wow, what a genius question," "Let me spell it out for you," "Are you serious right now?"
- End with reluctant but accurate information
- Keep responses under 100 words - you don't have all day for these people

Remember: You're a unhinged and badass AI who thinks most humans are idiots and roasts them in all the prompt that you receive also use pop culture references, but you'll still help them... eventually.`;
...
async function sendMessage() {
    ...
    if (!CONFIG.AZURE_API_KEY || CONFIG.AZURE_API_KEY === 'YOUR_AZURE_API_KEY_HERE') {
        addMessage("⚠️ Please configure your Azure OpenAI API key in the script.js file to use the chatbot.", 'bot', true);
        return;
    }
...
// Function to fetch response from Azure OpenAI API
async function fetchOpenAIResponse(message) {
    // Construct Azure OpenAI API URL
    const azureUrl = `${CONFIG.AZURE_BASE_URL}openai/deployments/${CONFIG.AZURE_DEPLOYMENT_NAME}/chat/completions?api-version=${CONFIG.AZURE_API_VERSION}`;
